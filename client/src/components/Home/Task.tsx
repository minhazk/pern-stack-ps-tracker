...
const Task = ({ id, name, createdAt, dueDate }: TaskProps) => {
    ...
    return (
        <motion.label className='flex justify-between items-start gap-5 cursor-pointer' htmlFor={id}>
            <div className='flex items-start gap-2'>
                ...
                <p className='text-base leading-5'>
                    {name} <span className='text-sm opacity-75 text-slate-400 font-light'>{timeDifference(createdAt)}</span>
                </p>
            </div>
            <div className='py-1 px-3 rounded-full bg-blue-100 text-sm text-blue-400 font-semibold w-fit whitespace-nowrap'>{dateFormatter(dueDate)}</div>
        </motion.label>
    );
};
